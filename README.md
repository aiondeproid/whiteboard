...
工場向けデジタル掲示板の編集アプリ。部（総務課・品質管理・製造・試作）ごとに PDF 資料を追加・削除・公開/非公開切り替えできる。社内共有の単一ボードを、編集者招待コードで参加した複数の編集アカウントが共同編集する。表示アプリ側は別の参加コードを入力するとボードに viewer として参加し、公開中の資料を閲覧できる。
...
- ボードは社内で 1 つだけ。編集アカウントは編集者招待コードでそのボードに editor として参加し、表示アカウントは別の参加コードで viewer として参加する
...
- 編集者招待コードを入力してボードに editor として参加、参加コードの発行・表示
