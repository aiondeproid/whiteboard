...
**単一ボード + 招待コード方式**（社内利用のため、ボードは会社で 1 つだけ）:

1. ボードはあらかじめ 1 つだけ用意されている（編集アプリ側にボードを新規作成する導線はない）
2. 編集アカウントがログインし、**編集者招待コード**を入力するとそのボードに **editor** として参加する（複数アカウントが editor になれる）
3. 表示アカウントがログインし、**参加コード**（editor 招待コードとは別のコード）を入力するとボードに **viewer** として参加
...
- 編集者招待コードと表示用参加コードは別物。表示用コードだけでは編集権限は取れない
...
flowchart TD
  ...
  Board --> Dept
  Dept --> Doc
  EditApp[Edit_App] -->|editor_via_invite_code_multiple| Board
...
1. ログイン
2. 編集者招待コード入力（未参加の場合。ボードは社内で 1 つのみ）
3. **資料ホーム**: 4 部から選択
